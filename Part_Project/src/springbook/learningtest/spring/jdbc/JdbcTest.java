...
public class JdbcTest {
	...
	public void simeJdbcTemplate() {
		...
		m.put("point", 3.5);
		dao.insert(m);
		dao.insert(new MapSqlParameterSource().addValue("id", 2)
				.addValue("name", "Book").addValue("point", 10.1));
		...
		dao.simpleJdbcTemplate.batchUpdate(
				"update member set name = :name where id = :id",
				new SqlParameterSource[] {
						new MapSqlParameterSource().addValue("id", 1).addValue(
								"name", "Spring3"),
						new BeanPropertySqlParameterSource(new Member(2,
								"Book3", 0)) });
	...
	static class SimpleDao {
		...
		public void updates(Map<String, Object>[] maps) {
			this.simpleJdbcTemplate.batchUpdate(
					"update member set name = :name where id = :id", maps);
		}

		public Map<String, Object> getMap(int id) {
			return this.simpleJdbcTemplate.queryForMap(
		...
		public double point(int id) {
			return this.simpleJdbcTemplate.queryForObject(
					"select point from member where id = ?", Double.class, id);
		}

		public int rowCount() {
			return this.simpleJdbcTemplate
					.queryForInt("select count(*) from member");
		}

		public int rowCount(double min) {
			return this.simpleJdbcTemplate.queryForInt(
					"select count(*) from member where point > :min",
		...
		public void insert(Map map) {
			this.simpleJdbcTemplate
					.update("INSERT INTO MEMBER(ID, NAME, POINT) VALUES(:id, :name, :point)",
							map);
		}
	...
	public void simpleJdbcInsert() {
		...
		SimpleJdbcInsert memberInsert = new SimpleJdbcInsert(dataSource)
	...
	public void simpleJdbcInsertWithGeneratedKey() {
		...
		SimpleJdbcInsert registerInsert = new SimpleJdbcInsert(dataSource)
				.withTableName("register").usingGeneratedKeyColumns("id");
	...
	public void simpleJdbcInsertWithSqlParamSource() {
		...
		MapSqlParameterSource paramSource = new MapSqlParameterSource()
				.addValue("id", 1).addValue("name", "Spring")
				.addValue("point", 10.5);
		...
		List<Map<String, Object>> list = sjt
				.queryForList("select * from member order by id");
